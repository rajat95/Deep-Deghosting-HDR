...
def deepfuse_triple_2encoder(inp1, inp2, inp3):
    layers = tf.layers
    leaky_relu=tf.nn.leaky_relu
    with tf.variable_scope('DeepFuse'):
        ...
        merge=tf.concat([neg_merge1,neg_merge2, pos_merge1, pos_merge2],axis=-1)
        ...
        merge = tf.concat([conv, tf.concat([neg_pooled_feats1[-1],neg_pooled_feats2[-1], pos_pooled_feats1[-1], pos_pooled_feats2[-1] ],axis=-1)], axis=-1)
        ...
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_pooled_feats1[-2],neg_pooled_feats2[-2],pos_pooled_feats1[-2],pos_pooled_feats2[-2] ],axis=-1)], axis=-1)
        ...
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_pooled_feats1[-3],neg_pooled_feats2[-3], pos_pooled_feats1[-3],pos_pooled_feats2[-3]],axis=-1)], axis=-1)
        ...
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_pooled_feats1[-4],neg_pooled_feats2[-4], pos_pooled_feats1[-4], pos_pooled_feats2[-4]],axis=-1)], axis=-1)
...
#Extension of above to arbitrary length sequence, To be used for inference, concatenate inputs along batch axis
def deepfuse_gen_2encoder(neg_inp, pos_inp):
    layers = tf.layers
    leaky_relu=tf.nn.leaky_relu
    with tf.variable_scope('DeepFuse', reuse=tf.AUTO_REUSE):
        ...
        merged_encoding = tf.concat([tf.reduce_max(neg_encoding, axis=0, keepdims=True),
                                     ...
                                     tf.reduce_mean(pos_encoding, axis=0, keepdims=True),], axis=-1)
        conv = layers.conv2d(merged_encoding, 256, kernel_size = 3, strides = 1, padding = 'SAME', activation = leaky_relu,
                                                    kernel_initializer = tf.contrib.layers.variance_scaling_initializer(), name = 'dec_conv1')
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_feats[-1][0], neg_feats[-1][1], pos_feats[-1][0], pos_feats[-1][1]],axis=-1)], axis=-1)
        conv = layers.conv2d(merge, 128, kernel_size = 3, strides = 1, padding = 'SAME', activation = leaky_relu,
                                                    kernel_initializer = tf.contrib.layers.variance_scaling_initializer(), name = 'dec_conv2')
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_feats[-2][0], neg_feats[-2][1], pos_feats[-2][0], pos_feats[-2][1]],axis=-1)],axis=-1)

        conv = layers.conv2d(merge, 64, kernel_size = 3, strides = 1, padding = 'SAME', activation = leaky_relu,
                                                            kernel_initializer = tf.contrib.layers.variance_scaling_initializer(), name = 'dec_conv3')
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_feats[-3][0], neg_feats[-3][1], pos_feats[-3][0], pos_feats[-3][1]],axis=-1)],axis=-1)
        conv = layers.conv2d(merge, 32, kernel_size = 3, strides = 1, padding = 'SAME', activation = leaky_relu,
                                                            kernel_initializer = tf.contrib.layers.variance_scaling_initializer(), name = 'dec_conv4')
        conv = tf.keras.layers.UpSampling2D((2,2))(conv)
        merge = tf.concat([conv, tf.concat([neg_feats[-4][0], neg_feats[-4][1], pos_feats[-4][0], pos_feats[-4][1]],axis=-1)],axis=-1)
        conv = layers.conv2d(merge,3,kernel_size=3,strides=1,padding='SAME',activation=tf.nn.sigmoid,
                                            kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),name='fusion_output')
        return tf.clip_by_value(conv, 0.0, 1.0)
